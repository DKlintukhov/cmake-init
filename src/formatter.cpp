...
std::string Formatter::format_project() const {
  ...
  ss << "    message(FATAL_ERROR \"In-source builds are not allowed. You should create separate "
...
std::string Formatter::format_options() const {
  std::stringstream ss;
  if (ctx_.enable_testing())
    ss << "option(ENABLE_TESTS \"Enable tests\" OFF)\n";
  if (ctx_.enable_clang_tidy())
    ss << "option(ENABLE_CLANG_TIDY \"Enable Clang-Tidy\" OFF)\n";
  if (ctx_.enable_cppcheck())
    ss << "option(ENABLE_CPPCHECK \"Enable Cppcheck\" OFF)\n";
  return ss.str();
}
